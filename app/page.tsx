...
// Lägg till din tidszon här (Stockholm/Köpenhamn har samma tid)
...
function calculateDailyStats(logs: TimeLog[]) {
  ...
  return {
    ...
    // Tvinga rätt tidszon vid utskrift av klockslag
    startTime: new Date(firstIn.timestamp).toLocaleTimeString('sv-SE', { hour: '2-digit', minute: '2-digit', timeZone: TIMEZONE }),
    endTime: new Date(lastOut.timestamp).toLocaleTimeString('sv-SE', { hour: '2-digit', minute: '2-digit', timeZone: TIMEZONE }),
  };
}
...
export default async function Dashboard() {
  ...
  // Tvinga rätt tidszon även för datumet
    ...
    <main className="max-w-2xl mx-auto p-6 mt-10">
      ...
      <div className="space-y-6">
        {Object.entries(logsByDate).map(([date, dayLogs]) => {
          ...
          return (
            <div key={date} className="bg-gray-50 text-green-400 border p-5 rounded-xl shadow-sm">
